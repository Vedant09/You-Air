import {
  ...
  picking_default,
  project32_default
} from "./chunk-IKVJNRD3.js";
import "./chunk-VSIY6N4U.js";
...
var ArcLayer = class extends layer_default {
  ...
  _getModel() {
    return new Model(this.context.device, __spreadProps(__spreadValues({}, this.getShaders()), {
      id: this.props.id,
...
var BitmapLayer = class extends layer_default {
  ...
  updateState({ props, oldProps, changeFlags }) {
    ...
    if (props.bounds !== oldProps.bounds) {
      ...
      this.setState(__spreadValues({ mesh }, this._getCoordinateUniforms()));
  ...
  _updateAutoHighlight(info) {
    super._updateAutoHighlight(__spreadProps(__spreadValues({}, info), {
...
function buildRowMapping(mapping, columns, yOffset) {
  for (let i = 0; i < columns.length; i++) {
    ...
    mapping[id] = __spreadProps(__spreadValues({}, icon), {
      x: xOffset,
      y: yOffset
    });
  }
...
function getDiffIcons(data, getIcon, cachedIcons) {
  ...
  for (const object of iterable) {
    ...
    if (!icons[id] && (!cachedIcons[id] || icon.url !== cachedIcons[id].url)) {
      icons[id] = __spreadProps(__spreadValues({}, icon), { source: object, sourceIndex: objectInfo.index });
    }
  }
  return icons;
}
...
var LineLayer = class extends layer_default {
  ...
  draw({ uniforms }) {
    ...
    model.draw(this.context.renderPass);
    if (wrapLongitude) {
      model.shaderInputs.setProps({ line: __spreadProps(__spreadValues({}, lineProps), { useShortestPath: -1 }) });
      model.draw(this.context.renderPass);
    }
  }
  _getModel() {
...
var ColumnLayer = class extends layer_default {
  ...
  _getModels() {
    ...
    const fillModel = new Model(this.context.device, __spreadProps(__spreadValues({}, shaders), {
      id: `${this.props.id}-fill`,
      bufferLayout,
      isInstanced: true
    }));
    const wireframeModel = new Model(this.context.device, __spreadProps(__spreadValues({}, shaders), {
  ...
  draw({ uniforms }) {
    ...
    if (extruded && wireframe) {
      wireframeModel.shaderInputs.setProps({
        column: __spreadProps(__spreadValues({}, columnProps), {
          isStroke: true
        })
      });
...
var SolidPolygonLayer = class extends layer_default {
  ...
  draw({ uniforms }) {
    ...
    if (wireframeModel && wireframe) {
      ...
      wireframeModel.shaderInputs.setProps({ solidPolygon: __spreadProps(__spreadValues({}, renderUniforms), { isWireframe: true }) });
      wireframeModel.draw(this.context.renderPass);
    }
    if (sideModel && filled) {
  ...
  _getModels() {
    ...
    if (filled) {
      ...
      topModel = new Model(this.context.device, __spreadProps(__spreadValues({}, shaders), {
        id: `${id}-top`,
        topology: "triangle-list",
        bufferLayout,
        isIndexed: true,
    ...
    if (extruded) {
      ...
      sideModel = new Model(this.context.device, __spreadProps(__spreadValues({}, this.getShaders("side")), {
        id: `${id}-side`,
        bufferLayout,
        geometry: new Geometry({
          topology: "triangle-strip",
      ...
      wireframeModel = new Model(this.context.device, __spreadProps(__spreadValues({}, this.getShaders("side")), {
        id: `${id}-wireframe`,
        bufferLayout,
        geometry: new Geometry({
          topology: "line-strip",
...
function getPropertiesForIndex(data, propertiesIndex, numericPropsIndex) {
  const feature = {
    properties: __spreadValues({}, data.properties[propertiesIndex])
  };
...
var MultiIconLayer = class extends icon_layer_default {
  getShaders() {
    ...
    return __spreadProps(__spreadValues({}, shaders), { modules: [...shaders.modules, textUniforms, sdfUniforms], vs: multi_icon_layer_vertex_glsl_default, fs: multi_icon_layer_fragment_glsl_default });
  }
  initializeState() {
  ...
  draw(params) {
    ...
    if (sdf && outlineWidth) {
      ...
      const iconsTexture = iconManager.getTexture();
      if (iconsTexture) {
        model.shaderInputs.setProps({ sdf: __spreadProps(__spreadValues({}, sdfProps), { outlineBuffer: DEFAULT_BUFFER2 }) });
        model.draw(this.context.renderPass);
      }
    }
  }
...
var FontAtlasManager = class {
  constructor() {
    this.props = __spreadValues({}, DEFAULT_FONT_SETTINGS);
  }
  get atlas() {
    return this._atlas;
  }
  ...
  _generateFontAtlas(characterSet, cachedFontAtlas) {
    ...
    const { mapping, canvasHeight, xOffset, yOffsetMin, yOffsetMax } = buildMapping2(__spreadValues({
...
var TextLayer = class extends composite_layer_default {
  ...
  _updateFontAtlas() {
    ...
    const fontProps = __spreadProps(__spreadValues({}, fontSettings), {
      characterSet,
      fontFamily,
      fontWeight,
      _getFontRenderer
  ...
  _updateText() {
    ...
    if (textBuffer && startIndices) {
      const { texts, characterCount } = getTextFromBuffer(__spreadProps(__spreadValues({}, ArrayBuffer.isView(textBuffer) ? { value: textBuffer } : textBuffer), {
...
function createLayerPropsFromBinary(geojsonBinary, encodePickingColor) {
  ...
  layerProps.points.data = {
    ...
    attributes: __spreadProps(__spreadValues({}, points.attributes), {
  ...
  layerProps.lines.data = {
    ...
    attributes: __spreadProps(__spreadValues({}, lines.attributes), {
  ...
  layerProps.polygons.data = {
    ...
    attributes: __spreadProps(__spreadValues({}, polygons.attributes), {
...
var defaultProps15 = __spreadProps(__spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues({}, getDefaultProps(POINT_LAYER.circle)), getDefaultProps(POINT_LAYER.icon)), getDefaultProps(POINT_LAYER.text)), getDefaultProps(LINE_LAYER)), getDefaultProps(POLYGON_LAYER)), {
  ...
  _renderPointLayers() {
    ...
    for (const type of types) {
      ...
      if (PointsLayer) {
        ...
        if (type === "text" && binary) {
          const _a = pointsLayerProps.data.attributes, { instancePickingColors } = _a, rest = __objRest(_a, ["instancePickingColors"]);
          pointsLayerProps = __spreadProps(__spreadValues({}, pointsLayerProps), {
            // @ts-expect-error TODO - type binary data
            data: __spreadProps(__spreadValues({}, pointsLayerProps.data), { attributes: rest })
          });
        }
