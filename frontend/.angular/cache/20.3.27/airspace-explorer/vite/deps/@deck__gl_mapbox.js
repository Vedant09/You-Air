...
function getDeckInstance({ map, deck }) {
  ...
  const deckProps = __spreadProps(__spreadValues({}, deck.props), {
    _customRender: () => {
      map.triggerRepaint();
      customRender?.("");
    }
...
var MapboxOverlay = class {
    ...
    this._handleMouseEvent = (event) => {
      ...
      switch (mockEvent.type) {
        case "mousedown":
          ...
          this._lastMouseDownPoint = __spreadProps(__spreadValues({}, event.point), {
  ...
  filterProps(props) {
    const _a = props, { interleaved = false, useDevicePixels } = _a, deckProps = __objRest(_a, ["interleaved", "useDevicePixels"]);
  ...
  setProps(props) {
    ...
    if (this._deck && this._map) {
      this._deck.setProps(__spreadProps(__spreadValues({}, this._props), {
        views: this._getViews(this._map),
        parameters: __spreadValues(__spreadValues({}, getDefaultParameters(this._map, this._interleaved)), this._props.parameters)
      }));
    }
  }
  ...
  _onAddOverlaid(map) {
    ...
    this._deck = new deck_default(__spreadProps(__spreadValues({}, this._props), {
      parent: container,
      parameters: __spreadValues(__spreadValues({}, getDefaultParameters(map, false)), this._props.parameters),
      views: this._getViews(map),
      viewState: getViewState(map)
    }));
  ...
  _onAddInterleaved(map) {
    ...
    this._deck = getDeckInstance({
      map,
      deck: new deck_default(__spreadProps(__spreadValues({}, this._props), {
        views: this._getViews(map),
        gl,
        parameters: __spreadValues(__spreadValues({}, getDefaultParameters(map, true)), this._props.parameters)
      }))
    });
